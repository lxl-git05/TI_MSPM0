...
void Motor_A_Init(void)
{
    // ============ 参数配置 ============
    // Encoder参数
	Motor_A.Encoder = &Motor_A_Encoder;

	// PWM参数
	Motor_A.PWM = &MyPWM_Motor_A_IN1;
    ...
    // ============ 函数初始化 ============
    // Encoder初始化
	MyEncoder_Init(Motor_A.Encoder) ;
	MyPWM_Init(Motor_A.PWM);
...
void Motor_B_Init(void)
{
    // ============ 参数配置 ============
    // Encoder参数
	Motor_B.Encoder = &Motor_B_Encoder;

	// PWM参数
	Motor_B.PWM = &MyPWM_Motor_B_IN1;
    ...
    // ============ 函数初始化 ============
    // Encoder初始化
	MyEncoder_Init(Motor_B.Encoder) ;
	MyPWM_Init(Motor_B.PWM);
...
{
	// 得到总脉冲数
	int Motor_CNT = MyEncoder_Get_CNT(Motor->Encoder) * Motor->Encoder_Dir; // 修正方向所在
...
void GROUP1_IRQHandler(void)
{
    ...
    switch (DL_Interrupt_getPendingGroup(DL_INTERRUPT_GROUP_1))
    {
        case GPIO_MULTIPLE_GPIOB_INT_IIDX:
            MyEncoder_Counter_Tick(Motor_A.Encoder);
            MyEncoder_Counter_Tick(Motor_B.Encoder);
            break;
...
void Motor_SetPWM(Motor_Typedef *Motor , int PWM)
{
	...
	if (PWM >= 0)
	{
        ...
        MyPWM_SetCompare(Motor->PWM , (uint16_t)PWM ) ; // 变大	 ,差值变大
	}
	else
	{
        DL_GPIO_setPins(Motor->IN2_Port, Motor->IN2_Pin);   // 高电平
        MyPWM_SetCompare(Motor->PWM , (uint16_t)(Motor_MAX_PWM + PWM) ) ; // 变小	 ,差值变大
	}
}
